...
def print_confusion_matrix(predict,step,dirname):
    try:
        ...
        disp = ConfusionMatrixDisplay(confusion_matrix=d2_arr,display_labels=list(['Init','Attack','Benign']))
...
#    trainX = raw.loc[2000:, 1:]
    ...
    trainX = raw.values[10000:, 1:] # , , 7
    raw.values[:3000, 0] = 0
    trainy = raw.values[:3000,0]
    print(trainy)
    testX = raw.values[:10000, 1:]
    raw.values[3000:4000, 0] = 0
    testy = raw.values[3000:4000, 0]
...
def load_dataset(prefix=''):
    ...
    # 2Dto3D
    trainX = trainX.reshape(6000, 10,7)
    testX = testX.reshape(2000, 10, 7)
...
def evaluate_model(trainX, trainy, testX, testy):
    ...
    n_timesteps, n_features, n_outputs = 10, 7, 2
